...
void expect32(std::uint32_t addr, std::uint32_t expected) {
  ...
  if (actual != expected) {
    std::fprintf(stderr, "FB_AHB selfcheck FAIL: addr 0x%08x expected 0x%08x got 0x%08x\n",
...
// Verilated::dpiContext() sets the scope to fb_ahb_i before calling run_sim,
// so no manual scope management is needed here.
